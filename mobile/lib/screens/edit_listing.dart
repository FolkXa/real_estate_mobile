...
class EditListingScreen extends StatefulWidget {
  ...
  const EditListingScreen({Key? key, required this.realEstateId}) : super(key: key);
...
class _EditListingScreenState extends State<EditListingScreen> {
  ...
  final List<String> _estateTypes = ['คอนโด', 'บ้าน', 'ทาวน์เฮ้าส์', 'ที่ดิน', 'อพาร์ทเมนท์', 'วิลล่า'];
  ...
  Future<void> _loadData() async {
    try {
      ...
      // Load property data
      final property = await _firebaseService.getRealEstateById(widget.realEstateId);
      if (property == null) {
        setState(() {
      ...
      // Load images
      _existingImages = await _firebaseService.getImagesForRealEstate(widget.realEstateId);
  ...
  Future<void> _deleteImages() async {
    for (String imageUrl in _imagesToDelete) {
      try {
        ...
        if (snapshot.docs.isNotEmpty) {
          await _firestore.collection('image_real_estate').doc(snapshot.docs.first.id).delete();
  ...
  Future<List<String>> _uploadNewImages() async {
    ...
    for (var imageFile in _newImages) {
      final String fileName = 'estate_${widget.realEstateId}_${DateTime.now().millisecondsSinceEpoch}';
      ...
      body: _isInitialLoading
          ...
          : _currentUser == null
              ...
              : _currentUser!.role != 'worker'
                  ? const Center(child: Text('You do not have permission to edit listings'))
                                    ...
                                    itemBuilder: (context, index) {
                                      if (index < _existingImages.length) {
                                        ...
                                        return GestureDetector(
                                          onTap: () => _viewFullImage(_existingImages, index),
                                          child: Container(
                                            width: 100,
